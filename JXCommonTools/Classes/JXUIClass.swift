...
class JXAlertView: UIControl {
    ...
    lazy var titleLabel: UILabel = {
        let view = UILabel()
        view.font = UIFont(font: "PingFangSC-Medium", size: 17)
    ...
    lazy var messageLabel: UILabel = {
        let view = UILabel()
        view.font = UIFont(font: "PingFangSC-Medium", size: 14)
        view.textAlignment = .center
        view.numberOfLines = -1
        view.textColor = UIColor(hex: "#1A1A1A")
        return view
    ...
    lazy var cancelButton: UIButton = {
        ...
        view.setBackgroundImage(.image(color: .init(hex: "#E4E7F0"), size: .init(width: 120, height: 38), corners: .allCorners, radius: 19), for: .normal)
        view.addTarget(self, action: #selector(jxCancelButtonClick), for: .touchUpInside)
        return view
    }()
    ...
    lazy var confirmButton: UIButton = {
        ...
        view.setBackgroundImage(.gradient([UIColor(hex: "#F474A3"), UIColor(hex: "#EF90ED")], size: .init(width: 120, height: 38), radius: 19, locations: nil, direction: .leftOblique), for: .normal)
        view.addTarget(self, action: #selector(jxConfirmButtonClick), for: .touchUpInside)
        return view
    }()
    ...
    @objc fileprivate func jxCancelButtonClick() {
        clickBlock?(-1, self)
        removeFromSuperview()
    }

    @objc fileprivate func jxConfirmButtonClick() {
        clickBlock?(1, self)
        removeFromSuperview()
    }
    
    // MARK: - 设置文案
    fileprivate func jxSet(title: String?, message: String?, cancelTitle: String?, confirmTitle: String?) {
        titleLabel.text = title
        messageLabel.text = message
        cancelButton.setTitle(cancelTitle, for: .normal)
        confirmButton.setTitle(confirmTitle, for: .normal)
        
        let showButton = (confirmTitle?.length ?? 0 > 0 ||
                          cancelTitle?.length ?? 0 > 0 )
        
        if (title?.length ?? 0 == 0) {
            layoutTextOnlyMessage(showButton)
        } else if (message?.length ?? 0 == 0) {
        ...
        if (cancelTitle?.length ?? 0 == 0) {
            layoutButtonOnlyConfirm()
            return
        }
        if (confirmTitle?.length ?? 0 == 0) {
...
extension JXAlertView {
    ...
    public static func jx_show(toView: UIView?, title: String?, message: String?, block: ((_ index: Int, _ alert: JXAlertView)->())? ,cancelTitle: String? = "取消", confirmTitle: String? = "确认", aRemove: Bool = true) -> JXAlertView? {
        guard let onView = (toView == nil) ? JXWindow.keyWindow : toView else {
            print("[JXAlertView]未找到展示到哪")
            return nil
        }
        
        guard (title?.length ?? 0 > 0 ||
               message?.length ?? 0 > 0) else {
            print("[JXAlertView]没有展示内容")
            return nil
        }
        
        let alertView = JXAlertView(frame: UIScreen.main.bounds)
        alertView.aRemove = aRemove
        alertView.clickBlock = block
        alertView.jxSet(title: title, message: message, cancelTitle: cancelTitle, confirmTitle: confirmTitle)
...
class JXInsetsLabel: UILabel {
    ...
    var jx_insets = UIEdgeInsets.zero
...
class JXSlider: UISlider {
    ...
    lazy var jxLeftLabel: UILabel = {
        let view = UILabel()
        view.font = UIFont(font: "PingFangSC-Semibold", size: 10)
